"""
Frozen CodeBERT + Logistic Regression baseline for SemEval 2026 Task 13 – Subtask A.
...
Pipeline:
  ...
  7. Save the LR model and predictions under models/task_a_frozen_codebert_lr.
...
def prepare_task_a_for_features(
    data_root: str = "data",
) -> Tuple[Dataset, Dataset, Dataset]:
    """
    Load and preprocess Task A data for frozen CodeBERT feature extraction.
    ...
    train_ds = Dataset.from_pandas(
        train_df[["code", "label"]], preserve_index=False
    )
    val_ds = Dataset.from_pandas(
        val_df[["code", "label"]], preserve_index=False
    )
    test_ds = Dataset.from_pandas(
        test_df[["code"]], preserve_index=False
    )
...
def train_frozen_codebert_lr_task_a(
    data_root: str = "data",
    output_dir: str = "models/task_a_frozen_codebert_lr",
):
    """
    Train Logistic Regression on frozen CodeBERT features for Subtask A.

    Args:
        data_root: Root data directory where data/task_a lives.
    ...
    train_tok, val_tok, test_tok = prepare_task_a_for_features(
        data_root=data_root
    )
    ...
    print("Logistic Regression trained on frozen CodeBERT features (Task A).")
    ...
    print(f"[Task A] Frozen CodeBERT + LR Dev Accuracy : {val_acc:.4f}")
    print(f"[Task A] Frozen CodeBERT + LR Dev Macro F1 : {val_f1:.4f}")
...
if __name__ == "__main__":
    train_frozen_codebert_lr_task_a()
