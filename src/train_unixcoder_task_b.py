"""
Fine-tune UniXcoder for SemEval 2026 Task 13 – Subtask B (authorship detection).
...
This script:
  ...
  6. Saves the model, tokenizer, and test predictions under models/task_b_unixcoder.
"""
...
MODEL_NAME = "microsoft/unixcoder-base"  # change this if you used a different UniXcoder variant


def prepare_task_b_datasets_unixcoder(
    data_root: str = "data",
    max_length: int = 256,
) -> Dict[str, Dataset]:
    """
    Load and preprocess Task B data for UniXcoder fine-tuning.
...
def train_unixcoder_task_b(
    data_root: str = "data",
    output_dir: str = "models/task_b_unixcoder",
    ...
    """
    Fine-tune UniXcoder for Subtask B.
    ...
    # Prepare datasets
    ds = prepare_task_b_datasets_unixcoder(data_root=data_root, max_length=max_length)
    train_tok = ds["train"]
    val_tok   = ds["val"]
    test_tok  = ds["test"]
    ...
    print("UniXcoder Task B validation results:", eval_results)
    ...
    # Optional: get predictions on test set for later submission / ensemble
