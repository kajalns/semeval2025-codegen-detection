"""
Frozen CodeBERT + BiLSTM classifier for SemEval 2026 Task 13 – Subtask B.
...
Pipeline:
  ...
  8. Save the BiLSTM head weights and test predictions under models/task_b_frozen_codebert_bilstm.
"""
...
def prepare_task_b_datasets_bilstm(
    data_root: str = "data",
    max_length: int = 256,
) -> Dict[str, Dataset]:
    """
    Load and preprocess Task B data for the frozen CodeBERT + BiLSTM model.
...
def train_frozen_codebert_bilstm_task_b(
    data_root: str = "data",
    output_dir: str = "models/task_b_frozen_codebert_bilstm",
    ...
    """
    Train the frozen CodeBERT + BiLSTM classifier on Subtask B.
    ...
    #Prepare datasets
    ds = prepare_task_b_datasets_bilstm(data_root=data_root, max_length=max_length)
    train_tok = ds["train"]
    val_tok   = ds["val"]
    test_tok  = ds["test"]
    ...
    #We disable Trainer's save/load of "best model" to avoid save_pretrained issues
    ...
    y_val_true = train_tok.column_names  # we need actual labels from df
    # easier: re-read labels from val_tok
    y_val_true = val_tok["labels"]
    y_val_true = np.array(y_val_true, dtype=int)

    val_acc = accuracy_score(y_val_true, val_preds)
    val_f1  = f1_score(y_val_true, val_preds, average="macro")

    print(f"[Task B] Frozen CodeBERT + BiLSTM Dev Accuracy : {val_acc:.4f}")
    print(f"[Task B] Frozen CodeBERT + BiLSTM Dev Macro F1 : {val_f1:.4f}")
...
if __name__ == "__main__":
    train_frozen_codebert_bilstm_task_b()
