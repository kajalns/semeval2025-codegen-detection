...
def load_task_a_splits(data_root: str = "data") -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Load train, validation, and test splits for SemEval Task 13 Subtask A.

    This function assumes that the parquet files are stored under:

        data/task_a/task_a_training_set.parquet
        data/task_a/task_a_validation_set.parquet
        data/task_a/task_a_test_set_sample.parquet

    Args:
        data_root:
            Root directory where the 'task_a' folder is located.
            By default we use 'data', so the expected path is 'data/task_a'.

    Returns:
        A tuple (train_df, val_df, test_df) with the raw DataFrames.
    """
    base = Path(data_root) / "task_a"

    train_path = base / "task_a_training_set.parquet"
...
def basic_clean_task_a(df: pd.DataFrame) -> pd.DataFrame:
    """
    Apply basic cleaning for Subtask A:
...
if __name__ == "__main__":
    ...
    #   python -m src.data_utils_task_a
    train, val, test = load_task_a_splits(data_root="data")
    print("Task A shapes:")
