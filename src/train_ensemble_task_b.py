# src/train_ensemble_task_b.py

"""
Weighted-probability ensemble for SemEval 2026 Task 13 – Subtask B (authorship detection).

This script:
  1. Loads validation and test probabilities from individual models:
       - CodeBERT      → experiments/task_b_codebert/val_probs.npy, test_probs.npy
       - GraphCodeBERT → experiments/task_b_graphcodebert/val_probs.npy, test_probs.npy
       - UniXcoder     → experiments/task_b_unixcoder/val_probs.npy, test_probs.npy
  2. Loads the true validation labels from the Task B validation parquet file.
...
def ensemble_task_b(
    ...
    output_dir: str = "experiments/task_b_ensemble",
    ...
    """
    Build and evaluate a weighted-probability ensemble for Subtask B.
    ...
    _, val_df, test_df = load_task_b_splits(data_root=data_root)
    val_df = basic_clean_task_b(val_df)
    y_val = val_df["label"].astype(int).values

    #Load probability matrices from each model
    model_folders = {
        "codebert": "task_b_codebert",
        "graphcodebert": "task_b_graphcodebert",
        "unixcoder": "task_b_unixcoder",
    }
    ...
    print("\n=== Individual model performance on validation (Task B) ===")
    for name, probs in val_probs_dict.items():
    ...
    weights = np.array([weight_codebert, weight_graphcodebert, weight_unixcoder], dtype=float)
    ...
    sub_path = output_path / "submission_ensemble_task_b.csv"
