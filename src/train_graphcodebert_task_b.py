"""
Fine-tune GraphCodeBERT for SemEval 2026 Task 13 – Subtask B (authorship detection).

This script:
  1. Loads Task B train/validation/test splits from data/task_b.
  2. Applies basic cleaning: keep (code, label), drop missing code.
  3. Tokenizes code with the GraphCodeBERT tokenizer.
  4. Fine-tunes `microsoft/graphcodebert-base` for multi-class classification.
...
def prepare_task_b_datasets(
    ...
    """
    Load and preprocess Task B data for GraphCodeBERT fine-tuning.
...
def train_graphcodebert_task_b(
    data_root: str = "data",
    output_dir: str = "models/task_b_graphcodebert",
    ...
    """
    Fine-tune GraphCodeBERT for Subtask B.
